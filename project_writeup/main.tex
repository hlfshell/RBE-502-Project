...
\noindent Convergences under forces in the $n_3$ direction benefit from strong $Q_{(4,4)}$ and $Q_{(5,5)}$ but can have far lower penalties before not converging.  Note in Table \ref{tab:STFCF2} the proximity to home in $x$ and $y$ in the early scenarios (still $z$=2.8), while the state diverges at $Q_{(4,4)}=Q_{(5,5)}=145$.
\begin{table}[H] %[bp]
\begin{centering}
\begin{tabular}{|cccc|}
\hline
